...
class PriorityQueue:
    ...
    def push(self, value) :
        ...
        while index != 1 :
            if self.data[index//2] > self.data[index]: #부모노드가 더 크면
                self.data[index//2], self.data[index] = self.data[index], self.data[index//2] #서로 교환    
                index = index//2        
            else : #부모노드가 더 작으면
                break
    ...
    def pop(self) :
        ...
        if len(self.data) ==1 :
            return
        self.data[1] = self.data[-1] #루트노드랑 마지막노드 교환
        self.data.pop() #마지막 노드 제거
        index = 1 #루트노드
        while True : # 부모가 자식보다 작을때 까지 교환
            priority = -1 # 왼쪽 , 오른쪽중 누굴 선택할지 , 임시로 절대올수없는값 넣기
            l_chil_idx = index*2
            r_chil_idx = index*2 +1
            # 1. 왼쪽 자식노드가 없는 경우 , 오른쪽 노드도 무조건 없다. 그러므로 리프노드
            if len(self.data) - 1 < l_chil_idx: #전체 노드수보다 왼쪽노드번호가 클수 없으므로
                break
            # 2. 왼쪽 자식만 있는경우
            elif len(self.data) -1 <r_chil_idx:
                priority = l_chil_idx # 왼쪽을 바꿔줄 후보로 넣어둠
            # 3. 두 자식다 있는경우
            else:
                if self.data[l_chil_idx] < self.data[r_chil_idx]: #둘중 작은자식 후보로
                    priority = l_chil_idx
                else :
                    priority = r_chil_idx
            if self.data[index] > self.data[priority]:
                self.data[index], self.data[priority] = self.data[priority], self.data[index] #서로 교환
                index= priority
            else :
                break
